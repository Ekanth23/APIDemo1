...
//		.asString();
	...
	System.out.println(gc.getCourses().getApi().get(1).getCoureseTitle()); 
	
	List<API> apicourse = gc.getCourses().getApi();
	for (int i=0; i<apicourse.size(); i++)
	{
		if(apicourse.get(i).getCoureseTitle().equalsIgnoreCase("SoapUI Webservices testing"))
		{
	...
	{
		a.add(webAutomation.get(j).getCoureseTitle());
